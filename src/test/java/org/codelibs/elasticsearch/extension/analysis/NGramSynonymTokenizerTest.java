...
import org.codelibs.elasticsearch.extension.analysis.NGramSynonymTokenizer.MyToken;
...
public class NGramSynonymTokenizerTest {
    ...
    public void testGetNextBlockLong() throws Exception {
        ...
        assertBlocks(tokenizer, new int[] { 0, NGramSynonymTokenizer.BUFFER_SIZE },
                src1.substring(0, NGramSynonymTokenizer.BUFFER_SIZE - 1), src2.substring(0, NGramSynonymTokenizer.BUFFER_SIZE - 2));

        src1 = getLengthDummyBlock(NGramSynonymTokenizer.BUFFER_SIZE, 'a', ' ');
        src2 = getLengthDummyBlock(NGramSynonymTokenizer.BUFFER_SIZE - 2, 'a', ' ');
        tokenizer = getTokenizer(src1 + src2);
        assertBlocks(tokenizer, new int[] { 0, NGramSynonymTokenizer.BUFFER_SIZE + 1 },
                src1.substring(0, NGramSynonymTokenizer.BUFFER_SIZE), src2.substring(0, NGramSynonymTokenizer.BUFFER_SIZE - 2));
    ...
    public void testSingleSynonym() throws Exception {
        ...
        stream.reset();
        assertTokenStream(stream, "a,0,2,1");
    ...
    public void testMultipleSynonyms() throws Exception {
        ...
        stream.reset();
        assertTokenStream(stream, "a,0,1,1/b,1,2,1/a,2,3,1/b,3,5,1");
    ...
    public void testAfterStrSingleSynonym2() throws Exception {
        ...
        stream = a.tokenStream("f", new StringReader("abcd"));
        stream.reset();
        assertTokenStream(stream, "a,0,1,1/b,1,2,1/bc,1,3,0/cd,2,4,1");

        a = new NGramSynonymTokenizerTestAnalyzer(2, false, "a,aa");
        stream = a.tokenStream("f", new StringReader("abcde"));
        stream.reset();
        assertTokenStream(stream, "a,0,1,1/b,1,2,1/bc,1,3,0/cd,2,4,1/de,3,5,1");
    }
    ...
    public void testSandwichStr2() throws Exception {
        ...
        stream = a.tokenStream("f", new StringReader("abba"));
        stream.reset();
        assertTokenStream(stream, "a,0,1,1/b,1,2,1/bb,1,3,0/b,2,3,0/a,3,4,1");

        a = new NGramSynonymTokenizerTestAnalyzer(2, false, "a,aa");
        stream = a.tokenStream("f", new StringReader("abcda"));
        stream.reset();
        assertTokenStream(stream, "a,0,1,1/b,1,2,1/bc,1,3,0/cd,2,4,1/d,3,4,0/a,4,5,1");

        a = new NGramSynonymTokenizerTestAnalyzer(2, false, "a,aa");
        stream = a.tokenStream("f", new StringReader("abcdea"));
        stream.reset();
        assertTokenStream(stream, "a,0,1,1/b,1,2,1/bc,1,3,0/cd,2,4,1/de,3,5,1/e,4,5,0/a,5,6,1");
    }
    ...
    public void testSandwichStr3() throws Exception {
        ...
        stream = a.tokenStream("f", new StringReader("abcda"));
        stream.reset();
        assertTokenStream(stream, "a,0,1,1/b,1,2,1/bc,1,3,0/bcd,1,4,0/cd,2,4,0/d,3,4,0/a,4,5,1");

        a = new NGramSynonymTokenizerTestAnalyzer(3, false, "a,aa");
        stream = a.tokenStream("f", new StringReader("abcdea"));
        stream.reset();
        assertTokenStream(stream, "a,0,1,1/b,1,2,1/bc,1,3,0/bcd,1,4,0/cde,2,5,1/de,3,5,0/e,4,5,0/a,5,6,1");

        a = new NGramSynonymTokenizerTestAnalyzer(3, false, "a,aa");
        stream = a.tokenStream("f", new StringReader("abcdefa"));
        stream.reset();
        assertTokenStream(stream, "a,0,1,1/b,1,2,1/bc,1,3,0/bcd,1,4,0/cde,2,5,1/def,3,6,1/ef,4,6,0/f,5,6,0/a,6,7,1");
    }
    ...
    public void testSandwichStr4() throws Exception {
        ...
        stream = a.tokenStream("f", new StringReader("abcdea"));
        stream.reset();
        assertTokenStream(stream, "a,0,1,1/b,1,2,1/bc,1,3,0/bcd,1,4,0/bcde,1,5,0/cde,2,5,0/de,3,5,0/e,4,5,0/a,5,6,1");

        a = new NGramSynonymTokenizerTestAnalyzer(4, false, "a,aa");
        stream = a.tokenStream("f", new StringReader("abcdefa"));
        stream.reset();
        assertTokenStream(stream, "a,0,1,1/b,1,2,1/bc,1,3,0/bcd,1,4,0/bcde,1,5,0/cdef,2,6,1/def,3,6,0/ef,4,6,0/f,5,6,0/a,6,7,1");
    }
    ...
    public void testSandwichSynonym3() throws Exception {
        ...
        stream.reset();
        assertTokenStream(stream,
                "fed,0,3,1/edc,1,4,1/dcb,2,5,1/cb,3,5,0/b,4,5,0/a,5,6,1/b,6,7,1/bc,6,8,0/bcd,6,9,0/cde,7,10,1/def,8,11,1");
    }
    ...
    public void testSandwichSynonym4() throws Exception {
        ...
        stream.reset();
        assertTokenStream(stream,
                "fedc,0,4,1/edcb,1,5,1/dcb,2,5,0/cb,3,5,0/b,4,5,0/a,5,6,1/b,6,7,1/bc,6,8,0/bcd,6,9,0/bcde,6,10,0/cdef,7,11,1");
    }
    ...
    public void testComplex2() throws Exception {
        ...
        stream = a.tokenStream("f", new StringReader("edcabcdea"));
        stream.reset();
        assertTokenStream(stream, "ed,0,2,1/dc,1,3,1/c,2,3,0/a,3,4,1/b,4,5,1/c,5,6,1/cd,5,7,0/de,6,8,1/e,7,8,0/a");

        a = new NGramSynonymTokenizerTestAnalyzer(2, false, "a,aa/b,bb");
        stream = a.tokenStream("f", new StringReader("fedcabcdefa"));
        stream.reset();
        assertTokenStream(stream,
                "fe,0,2,1/ed,1,3,1/dc,2,4,1/c,3,4,0/a,4,5,1/b,5,6,1/c,6,7,1/cd,6,8,0/de,7,9,1/ef,8,10,1/f,9,10,0/a,10,11,1");
    }
    ...
    public void testComplex3() throws Exception {
        ...
        stream.reset();
        assertTokenStream(stream,
                "fed,0,3,1/edc,1,4,1/dc,2,4,0/c,3,4,0/a,4,5,1/b,5,6,1/c,6,7,1/cd,6,8,0/cde,6,9,0/def,7,10,1/ef,8,10,0/f,9,10,0/a,10,11,1");

        a = new NGramSynonymTokenizerTestAnalyzer(3, false, "a,aa/b,bb");
        stream = a.tokenStream("f", new StringReader("gfedcabcdefga"));
        stream.reset();
        assertTokenStream(stream,
                "gfe,0,3,1/fed,1,4,1/edc,2,5,1/dc,3,5,0/c,4,5,0/a,5,6,1/b,6,7,1/c,7,8,1/cd,7,9,0/cde,7,10,0/def,8,11,1/efg,9,12,1/fg,10,12,0/g,11,12,0/a,12,13,1");
    }
    ...
    public void testComplex4() throws Exception {
        ...
        stream.reset();
        assertTokenStream(stream,
                "fedc,0,4,1/edc,1,4,0/dc,2,4,0/c,3,4,0/a,4,5,1/b,5,6,1/c,6,7,1/cd,6,8,0/cde,6,9,0/cdef,6,10,0/def,7,10,0/ef,8,10,0/f,9,10,0/a,10,11,1");

        a = new NGramSynonymTokenizerTestAnalyzer(4, false, "a,aa/b,bb");
        stream = a.tokenStream("f", new StringReader("gfedcabcdefga"));
        stream.reset();
        assertTokenStream(stream,
                "gfed,0,4,1/fedc,1,5,1/edc,2,5,0/dc,3,5,0/c,4,5,0/a,5,6,1/b,6,7,1/c,7,8,1/cd,7,9,0/cde,7,10,0/cdef,7,11,0/defg,8,12,1/efg,9,12,0/fg,10,12,0/g,11,12,0/a,12,13,1");
    }
    ...
    public static final class NGramSynonymTokenizerTestAnalyzer extends Analyzer {
        ...
        protected TokenStreamComponents createComponents(String fieldName) {
            final Tokenizer source =
                    new NGramSynonymTokenizer(n, delimiters, expand, expandNgram, true, new SynonymLoader(null, null, expand, null) {
                        @Override
